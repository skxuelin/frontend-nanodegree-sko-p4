...
var resizePizzas = function(size) { 
  ...
  /*SKO this commented out section of codes are changed to the ones below
  
  // Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
  /*function determineDx (elem, size) {
    var oldwidth = elem.offsetWidth;
    //var windowwidth = document.querySelector("#randomPizzas").offsetWidth;
    //console.log("function windowwidth:" + windowwidth);
  ...
  //SKO: This is added from the old determineDx function. 
  function sizeSwitcher (size) {
      switch(size) {
        case "1":
          return 0.25;
  ...
  /*SKO: Changes made into optimizing changePizzaSizes:
  1. Move all variables that do not need to be in the loop outside the loop:
  windowwidth and newsize only need to be pulled once.
  2. To calculate the new width per pizza, we just need to calculate it based on what the new size
  should be, either 0.25, 0.3333, or 0.5 relative to the window width. We do not need to loop
  through to find out what is the current width and add it back on. This can be done outside the loop.
  */  
  function changePizzaSizes(size) {
    ...
    for (var i = 0; i < pizzaLength; i++) {
      var elem = document.getElementsByClassName("randomPizzaContainer")[i];
...
//SKO: see updated function below
...
//SKO: Changes made into this function are:
//1. Since there are only 5 different phases, take their creation out from the object loop so they are not calculated for every object. 
//Create an array to store 5 different phases
//2. Take out finding scroll top out from the loop since we only need to find it once
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");
  ...
  for (var i = 0; i < 5; i++) {
    var phase = Math.sin((scrollTop / 1250) + i);
    phases.push(phase);
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //SKO:added from the function below. Use the selector once instead of multiple times in a loop.
  ...
  for (var i = 0; i < 40; i++) {
    ...
    //SKO: change querySelector to getElementById and take it out of the loop
    //see movingPizzas1 variable above
